layui.use(['jquery','layer', 'table','form','laydate'], function() {
    ...
    function loadPageRoomSale(){
        ...
        table.render({  //数据表格的数据渲染(此UI框架底层是进行异步加载)
            ...
            , url: 'roomSale/loadDataByParams' //访问服务器端的数据接口(异步请求)，返回的json格式的数据
    ...
    table.on('tool(test)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'query') {
            //1.3.简单属性表单赋值回显
            form.val("roomSaleForm", { //formTest 即 class="layui-form" 所在元素属性 lay-filter="" 对应的值
                "id":data.id
            ...
            layer.open({
                type:1,  //弹出类型
                title:"查看用户消费详情界面",  //弹框标题
                area:['750px','600px'],  //弹框款高度
                anim: 4,  //弹出的动画效果
                shade:0.5,  //阴影遮罩
                content:$("#roomSaleDiv")  //弹出的内容
            });
        }else {
